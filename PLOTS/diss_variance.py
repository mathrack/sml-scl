...
class case(object):
    def __init__(self, xlsfile, ncell):
         ...
         self.sht = self.wkbk.sheet_by_name('diss_variance')
         self.y = purge(self.sht.col_values(1)[2:self.ny])
         self.L18dat = abs(purge(self.sht.col_values(2)[2:self.ny]))
         self.L18sig = 2*purge(self.sht.col_values(3)[2:self.ny])
         self.L18 = self.L18sig / (self.L18dat + self.L18sig)
         self.S18dat = abs(purge(self.sht.col_values(9)[2:self.ny]))
...
savefig("diss_variance.png",bbox_inches='tight')
savefig("diss_variance.pdf",bbox_inches='tight')
